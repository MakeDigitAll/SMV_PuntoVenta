...
const WarehouseOutputs = () => {
  ...
  async function loadTask() {
    try {
      const response = await fetch("https://localhost:4000/ListadoSalida");
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      filteredUsers = filteredUsers.filter((data) =>
        Array.from(statusFilter).includes(data.provedor)
      );
    }
  ...
  const renderCell = React.useCallback((data, columnKey) => {
    ...
    switch (columnKey) {
      ...
      case "SucursalAlmacen":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-small capitalize">{data.sucursalAlmacen}</p>
          </div>
        );
      case "Tipo":
        return (
        ...
        >
          <Breadcrumbs aria-label="breadcrumb" color="foreground">
            ...
            <Typography
              ...
              className="text-foreground"
            >
              <MdOutbox sx={{ mr: 0.5 }} fontSize="inherit" />
              Salidas de Almacén
            </Typography>
          </Breadcrumbs>
        </div>
      ...
      <div className="py-2 px-2 flex justify-between items-center">
        <span className="w-[30%] text-small text-default-400">
          <span style={{ marginRight: "30px" }}>
            {data.length} Salidas de Almacén en total
          </span>
          {selectedKeys === "all"
            ? "All items selected"
  ...
  return (
    <div style={{ marginLeft: "40px", marginRight: "40px" }}>
      ...
      >
        ...
        <TableBody
          emptyContent={"No se encuentran Salidas de Almacén"}
          items={sortedItems}
        >
          {(item) => (
...
export default WarehouseOutputs;