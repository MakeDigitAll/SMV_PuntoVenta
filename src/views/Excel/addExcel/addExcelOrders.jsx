...
const AddExcelOrders = () => {
  const headers = [
    ...
    'Razon Social', 'RFC', 'Monto', 'Saldo', 'Surtido'
  ];
  ...
  const handleDownloadTemplate = async () => {
    ...
    const worksheet = workbook.addWorksheet('Orders template');
  ...
  const handleSaveToDatabase = async () => {
    try {
      const datosAEnviar = data.map(row => ({
        ...
        cotizacion: row[2] !== ' ' ? row[2] : null,
        numeroCliente: row[3] !== ' ' ? row[3] : null,
        cliente: row[4] !== ' ' ? row[4] : null,
        razonSocial: row[5] !== ' ' ? row[5] : null,
        rfc: row[6] !== ' ' ? row[6] : null,
        monto: row[7] !== ' ' ? row[7] : null,
        saldo: row[8] !== ' ' ? row[8] : null,
        surtido: row[9] !== ' ' ? row[9] : null,
      }));
  ...
  return (
    <div style={{ position: 'relative', zIndex: '0' }}>
      <Button size="sm" color='success' onClick={() => setModalIsOpen(true)}>Subir Pedidos</Button>
      <Modal
        isOpen={modalIsOpen}
