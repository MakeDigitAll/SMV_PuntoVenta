...
import { MdAlignHorizontalCenter, MdArrowDropDown, MdCreditCard, MdFilterList, MdImportContacts, MdMonetizationOn, MdPriceChange, MdPriceCheck, MdSearch, MdSecurity } from "react-icons/md";
...
const SecuritProfiles = () => {
    ...
    const handleView = (id) => {
        ...
        async function viewModal() {
            try {
                const response = await fetch(`http://localhost:4000/PerfilesSeguridad/${id}`);
                const data = await response.json();
                if (response.ok) {
                    setTask({
                        id: data.id,
    ...
    async function handleSubmit(e) {
        ...
        if (!task.nombrePerfil) {
            newErrors.nombrePerfil = "El Campo Nombre del Perfil de Seguridad es Obligatorio";
    ...
    async function handleEditing(e) {
        ...
        async function edit() {
            try {
                ...
                const res = await fetch(`http://localhost:4000/PerfilesSeguridad/${datosListado.id}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                    },
                ...
                return (
                    <div className="relative flex justify-center items-center gap-2">
                        <Dropdown>
                            ...
                            <DropdownMenu>
                                <DropdownItem onPress={() => handleView(data.id)}>Ver Perfil de Seguridad</DropdownItem>
                                <DropdownItem onPress={() => handleEdit(data.id)}>Editar Perfil de Seguridad</DropdownItem>
                                <DropdownItem onPress={() => console.log("configurar accesos")}>Configurar Accesos</DropdownItem>
                                <DropdownItem className="text-danger" onPress={() => handleDisable(data.id)}>Deshabilitar Perfil de Seguridad</DropdownItem>
                            </DropdownMenu>
                        </Dropdown>
                    </div>
                );
                    ...
                    >
                        ...
                        <div className="flex flex-wrap place-content-end space-x-2">
                            ...
                            <Button
                                size="sm"
                                color="primary"
                                onPress={() => navigate("/Settings/SecuritProfiles/ConfigureAccess")}
                            >
                                Boton prueba 
                            </Button>
                            <Button
    ...
    return (
        <div style={{ marginLeft: "40px", marginRight: "40px" }}>
            ...
            >
                ...
                <TableBody emptyContent={"No Securit Profiles found"} items={sortedItems}>
                    {(item) => (
                        ...
                        <>
                            <form onChange={handleChange} onSubmit={modeModal == "create" ? handleSubmit : handleEditing} >
                                ...
                                <ModalFooter>
                                    ...
                                    {showButton && (
                                        <Button id="BtnGuardar" color="primary" onPress={console.log("este es el boton de guardar")}>
                                            Guardar
                                        </Button>
                                    )}
                                </ModalFooter>
