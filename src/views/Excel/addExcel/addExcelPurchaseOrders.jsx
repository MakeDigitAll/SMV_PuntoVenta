...
const AddExcelPurchaseOrders = () => {
  const headers = [
    ' ', 'Fecha Registrada', 'fecha de compra', 'fecha de entrega', 'Provedor',
    'Vendedor', 'Referencia', 'Total', 'Productos'
  ];
  ...
  const handleDownloadTemplate = async () => {
    ...
    const worksheet = workbook.addWorksheet('Purchase Orders template');
  ...
  const handleSaveToDatabase = async () => {
    try {
      const datosAEnviar = data.map(row => ({
        fechaRegistrada: row[1] !== ' ' ? row[1] : null,
        fechaCompra: row[2] !== ' ' ? row[2] : null,
        fechaEntrega: row[3] !== ' ' ? row[3] : null,
        provedor: row[4] !== ' ' ? row[4] : null,
        vendedor: row[5] !== ' ' ? row[5] : null,
        referencia: row[6] !== ' ' ? row[6] : null,
        total: row[7] !== ' ' ? row[7] : null,
        productos: row[8] !== ' ' ? row[8] : null,
      }));
      // Realiza un mapeo individual para cada valor
      const responseArray = await Promise.all(datosAEnviar.map(async (valor, index) => {
        const response = await fetch(`https://localhost:443/OrdenCompra/ListadoEntradas`, {
          method: 'POST',
  ...
  return (
    <div style={{ position: 'relative', zIndex: '0' }}>
      <Button size="sm" color='success' onClick={() => setModalIsOpen(true)}>Subir ordenes de compra</Button>
      <Modal
        isOpen={modalIsOpen}
...
export default AddExcelPurchaseOrders;