...
const AddExcelPays = () => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    const worksheet = workbook.addWorksheet('Payments template');
  ...
  const handleSaveToDatabase = async () => {
    try {
      const datosAEnviar = data.map(row => ({
        id: row[1] !== ' ' ? row[1] : null,
        folio: row[2] !== ' ' ? row[2] : null,
        status: row[3] !== ' ' ? row[3] : null,
  ...
  return (
    <div style={{ position: 'relative', zIndex: '0' }}>
      <Button size="sm" color="secondary" onClick={() => setModalIsOpen(true)}>Subir Pagos</Button>
      <Modal
        isOpen={modalIsOpen}
