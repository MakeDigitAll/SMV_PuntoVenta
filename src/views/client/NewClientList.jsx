...
import { MdAddHomeWork, MdBook, MdCamera, MdLocationCity, MdNumbers, MdOutlineLocationCity, MdOutlineNumbers, MdPeople, MdPeopleAlt, MdPerson, MdPostAdd, MdSettings, MdStreetview } from "react-icons/md";
   ...
   const [modalMode, setModalMode] = useState("create");
    const handleCreateClick = (nuevosDatos) => {
      ...
      async function loadDireccionesE(id) {
        ...
        try {
          const response = await fetch(`http://localhost:4000/ClientesDireccionEnvio/${id}`);
          const data = await response.json();
          setTask2({
            nombre: data.nombre,
            direccion: data.direccion,
      ...
      const handleChange2 = (e) =>
       ...
       formData.append("document", document);
            try {
                ...
                if (response.status == 200 ? true : false) {
                    toast.success("Dirección de envío creado correctamente", { theme: "colored" });
                    onClose(true);
                            ...
                            <div className="md:col-span-5 text-center">
                              <div style={{ paddingLeft: "90px" }}>
                                <Input
                                  ...
                                  variant="faded"
                                  error={
                                    validationErrors.numerodecliente !== ""
                                  }
                                  errorMessage={
                                    validationErrors.numerodecliente
                                  }
