...
const columns = [
  ...
  { name: "Existencia", uid: "Existencia", sortable: true },
  { name: "Ubicación", uid: "Ubicacion", sortable: true },
  { name: "Minímo", uid: "Minimo", sortable: true },
  { name: "Máximo", uid: "Maximo", sortable: true },
  { name: "Reorden", uid: "Reorden", sortable: true },
  { name: "Lotes", uid: "Lotes", sortable: true },
...
const InventoryWarehouse = () => {
  ...
  async function loadTask() {
    try {
      const response = await fetch("https://localhost:4000/inventarioXAlmacen");
    ...
    switch (columnKey) {
      ...
      case "Minimo":
        ...
        case "Reorden":
            return (
                  <div className="flex flex-col">
                    <p className="text-bold text-small capitalize">{data.reorden}</p>
                  </div>
                );
        case "Lotes":
             return (
                      <div className="flex flex-col">
                        <p className="text-bold text-small capitalize">{data.lotes}</p>
                      </div>
                    );
      case "Actions":
        return (
        ...
        >
          <Breadcrumbs aria-label="breadcrumb" color="foreground">
            ...
            <Typography
              ...
              className="text-foreground"
            >
              <MdStore sx={{ mr: 0.5 }} fontSize="inherit" />
              Inventario X Almacén
            </Typography>
          </Breadcrumbs>
        </div>
        <div
      ...
      <div className="py-2 px-2 flex justify-between items-center">
        <span className="w-[30%] text-small text-default-400">
          <span style={{ marginRight: "30px" }}>
            {data.length} InvXalmacén en total
          </span>
          {selectedKeys === "all"
            ? "All items selected"
  ...
  return (
    <div style={{ marginLeft: "40px", marginRight: "40px" }}>
      ...
      >
        ...
        <TableBody
          emptyContent={"No se encuentran invXalmacén"}
          items={sortedItems}
        >
          {(item) => (
